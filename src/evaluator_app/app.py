...
# --- 🔄 トップ・ナビゲーション（ジャンプ機能搭載） ---
st.markdown("### 🚀 クイック・ナビゲーター")
# 🌟 数値入力とスライダーで直感的にジャンプ
nav_col1, nav_col2, nav_col3 = st.columns([1, 3, 1])

with nav_col1:
    # 1-indexedで表示
    jump_idx = st.number_input("問題番号でジャンプ", min_value=1, max_value=len(df), value=current_idx + 1, step=1)
...
with nav_col2:
    # 全体を俯瞰できるスライダー
    slider_idx = st.slider("シークバー", 1, len(df), current_idx + 1)
    if slider_idx - 1 != current_idx:
        st.session_state.current_idx = slider_idx - 1
        st.rerun()
...
with nav_col3:
    ...
    with b_prev:
        if st.button("⬅️", key="nav_prev", use_container_width=True, disabled=(current_idx == 0)):
            st.session_state.current_idx -= 1
            st.rerun()
    with b_next:
        if st.button("➡️", key="nav_next", use_container_width=True, disabled=(current_idx == len(df) - 1)):
            st.session_state.current_idx += 1
            st.rerun()

st.markdown(f"<h4 style='text-align: center; color: #666;'>現在の解答: {current_idx + 1} / {len(df)} 件</h4>", unsafe_allow_html=True)
st.markdown("---")
...
if status == "Error":
    ...
    selected_model = st.radio("目標モデルを選択：", target_models, index=default_index, horizontal=True, key=f"radio_{current_idx}")
    ...
    with l_col1:
        ...
        if target_sympy:
            ...
            st.caption("正規化された数式")
            st.code(TEMPLATES_EXPR[selected_model], language="python")
    with l_col2:
        st.success("**学生の解答**")
        ...
        if student_sympy:
            ...
            st.code(student_norm, language="python")
...
new_eval = st.slider("評価値（-1:未評価, 0:完全一致）", -1, 15, curr_eval, key=f"eval_{current_idx}")
comment_val = str(row.get('コメント', '')) if not pd.isna(row.get('コメント', '')) else ""
new_comment = st.text_area("分析・目視コメント", value=comment_val, key=f"comment_{current_idx}")

st.markdown("---")
b_col1, b_col2, b_col3, b_col4 = st.columns([1, 1, 2, 1])

with b_col1:
    if st.button("⬅️ 前へ", key="prev_bottom", use_container_width=True, disabled=(current_idx == 0)):
        st.session_state.current_idx -= 1
        st.rerun()

with b_col2:
    if st.button("💾 保存のみ", key="save_only", use_container_width=True):
...
with b_col3:
    if st.button("🚀 保存して次へ ➡️", key="save_and_next", type="primary", use_container_width=True, disabled=(current_idx == len(df) - 1)):
        st.session_state.df.at[current_idx, '評価値'] = new_eval
        st.session_state.df.at[current_idx, 'コメント'] = new_comment
        st.session_state.df.to_csv(csv_path, index=False, encoding='utf-8-sig')
        st.session_state.current_idx += 1
...
with b_col4:
    if st.button("次へ ➡️", key="next_bottom", use_container_width=True, disabled=(current_idx == len(df) - 1)):
        st.session_state.current_idx += 1
        st.rerun()