...
# ★ここからパターンマッチング関連の設定を更新
# パターン定義
pattern_definitions = {
    # 累乗微分: 2*x, x*2 が含まれている
    '累乗微分': r'(?:2\*x|x\*2)',
    
    # 合成関数: 15*(3*x+1)^4*(x^2+2) または 5*3*(3*x+1)^4*(x^2+2) が含まれている
    '合成関数': r'(?:15|5\*3)\s*\*\s*\(3\*x\+1\)\^4\s*\*\s*\(x\^2\+2\)'
}
...
try:
    ...
    for idx, row_data in df_processed.iterrows():
        ...
        for pm_col_name, pattern_regex in pattern_definitions.items():
            if pd.notna(response_to_check) and re.search(pattern_regex, response_to_check):
                ...
                matched_any_defined_pattern_for_summary = True # いずれかのパターンにマッチしたらTrue
        ...
        if score_to_check == 10:
            if not matched_any_defined_pattern_for_summary:
                ...
                counts['PM_その他正解'] += 1
        # 「PM_その他誤答」のロジック (スコアが0の行かつ、どのパターンにもマッチしなかった場合)
    ...
    if not final_output_df.empty:
        # 出力ファイル名を「3-02」と分かるように変更
        output_csv_filename_for_this_file = f"HW03-02_pattern_analysis.csv" # ここを修正
