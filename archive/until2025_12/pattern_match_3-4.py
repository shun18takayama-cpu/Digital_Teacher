...
# 'extracted_ans1' 列に純粋な解答形式が格納されているため、
# 'pattern_for_ans_cor_extraction' は不要です。
# target_column_for_ans/cor は、df_processedで'extracted_ans1'として扱われるため、直接使用

target_column_for_score = '評点/10.00'
incorrect_score_thresholds = [0] # 不正解者リストの作成

# ★ここから最適なパターンマッチング関連の設定を更新
# 最適なパターン定義 (PM_Optimal_Full_Derivative を削除)
# 1. PM_合成関数(外): 3 * (e^-x + x^2)^2 の形
pattern_optimal_composite_outer = r'3\s*\*\s*\((?:exp\s*\(-x\)|e\s*\^\s*\(-x\))\s*\+\s*x\s*\^2\)\s*\^2\s*|\s*\((?:exp\s*\(-x\)|e\s*\^\s*\(-x\))\s*\+\s*x\s*\^2\)\s*\^2\s*\*\s*3'

# 2. PM_eの微分: -e^(-x) または -exp(-x) の形
pattern_optimal_exp_derivative = r'(?:-1\s*\*\s*|-)(?:exp\s*\(-x\)|e\s*\^\s*\(-x\))'

# 3. PM_累乗の微分: x*2 または 2*x の形
pattern_optimal_power_derivative = r'2\s*\*\s*x|x\s*\*\s*2'

# パターンとそれに対応する新しい列名 (日本語に簡潔化)
pattern_definitions = {
    'PM_合成関数(外)': pattern_optimal_composite_outer,
    'PM_eの微分': pattern_optimal_exp_derivative,
    'PM_累乗の微分': pattern_optimal_power_derivative,
    # 'PM_全体正解' はここから削除
}
...
try:
    ...
    if not final_output_df.empty:
        # 出力ファイル名を「3-4」と分かるように変更
        output_csv_filename_for_this_file = f"HW03-04_pattern_analysis_optimal_jp.csv" # ここを変更
