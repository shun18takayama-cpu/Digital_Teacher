...
# ★ここからパターンマッチング関連の設定を追加
# パターン定義 (前回の会話で定義したもの)
pattern_1 = r'5\s*\*\s*\(3\*x\s*\+\s*1\)\s*|\s*\(3\*x\s*\+\s*1\)\s*\*\s*5'
pattern_2 = r'\(3\*x\s*\+\s*1\)\s*\^4\s*\*\s*\(diff\(3\*x\s*\+\s*1\),x\)\s*|\(diff\(3\*x\s*\+\s*1\),x\)\s*\*\s*3\s*\*\s*\(3\*x\s*\+\s*1\)\s*\^4\s*|3\s*\*\s*\(3\*x\s*\+\s*1\)\s*\^4\s*|\(3\*x\s*\+\s*1\)\s*\^4\s*\*\s*3'
pattern_3 = r'15\s*\*\s*\(3\*x\s*\+\s*1\)\s*\^4\s*|\s*\(3\*x\s*\+\s*1\)\s*\^4\s*\*\s*15'
pattern_4 = r'\(3\*x\s*\+\s*1\)\s*\*\s*diff\(x\s*\^2\s*\+2\)\s*|\s*diff\(x\s*\^2\s*\+2\)\s*\*\s*\(3\*x\s*\+\s*1\)\s*|2\*x\s*\*\s*\(3\*x\s*\+\s*1\)\s*|\s*\(3\*x\s*\+\s*1\)\s*\*\s*2\*x'

# パターンとそれに対応する新しい列名
pattern_definitions = {
    'PM_指数関数の微分の成功': pattern_1,
    'PM_合成関数の微分成功': pattern_2,
    'PM_合成関数指数関数両方の成功': pattern_3,
    'PM_指数関数2の成功': pattern_4,
}

# 出力したい列のリストにパターンマッチング結果列を追加
output_columns_to_save = [
    '姓', '開始日時', '受験完了', '継続時間',
    '評点/10.00', 
    'extracted_ans1',
    'extracted_cor1',
] + list(pattern_definitions.keys()) + ['PM_パターンマッチせずに成功', 'PM_パターンマッチせずに誤答'] # ★新しい列名をここに追加
# ★ここまでパターンマッチング関連の設定を追加
...
for current_input_file_path in full_input_file_paths:
    ...
    try:
        ...
        for col_name in pattern_definitions.keys():
            incorrect_performers_all_answers_df[col_name] = ''
        incorrect_performers_all_answers_df['PM_パターンマッチせずに成功'] = ''
        incorrect_performers_all_answers_df['PM_パターンマッチせずに誤答'] = ''

        for idx, row_data in incorrect_performers_all_answers_df.iterrows():
            response_to_check = str(row_data['extracted_ans1'])
            score_to_check = row_data['評点/10.00']
            
            matched_any_defined_pattern = False # 指定されたいずれかのパターンにマッチしたか

            # 各パターンをチェックし、マッチした場合は「〇」を付ける
            for pm_col_name, pattern_regex in pattern_definitions.items():
                if pd.notna(response_to_check) and re.search(pattern_regex, response_to_check):
                    incorrect_performers_all_answers_df.at[idx, pm_col_name] = '〇'
                    matched_any_defined_pattern = True

            # パターンマッチせずに成功/誤答のロジック
            if score_to_check == 10:
                if not matched_any_defined_pattern:
                    incorrect_performers_all_answers_df.at[idx, 'PM_パターンマッチせずに成功'] = '〇'
            elif score_to_check == 0:
                if not matched_any_defined_pattern:
                    incorrect_performers_all_answers_df.at[idx, 'PM_パターンマッチせずに誤答'] = '〇'
        # --- ★ここまでパターンマッチング処理を統合 ---
        ...
        # -------------------------------------------------------------
        # パターンマッチング結果の列も確認対象に含める
        columns_to_check_for_excel = ['extracted_ans1', 'extracted_cor1'] + list(pattern_definitions.keys()) + ['PM_パターンマッチせずに成功', 'PM_パターンマッチせずに誤答']
