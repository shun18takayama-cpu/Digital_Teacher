...
# 最適なパターン定義 (ユーザーの最新の要求に合わせて更新)

# 各要素の基本的な正規表現（符号や結合記号を含まない）
# eの関数部分: e^(2*x) または exp(2*x)
e_base_pattern = r"(?:e\^\{?2\*?x\}?|exp\s*\(\s*2\*?x\s*\))"
# sinの関数部分: sin(x^3+1) または sin(1+x^3), 閉じ括弧は1つまたは2つ
sin_base_pattern = r"sin\s*\(\s*(?:x\^3\s*\+\s*1|1\s*\+\s*x\^3)\s*\)\)?"
# 累乗の関数部分: 3*x^2 または x^2*3
power_base_pattern = r"(?:3\*?x\^2|x\^2\*?3)"

# 項間の結合記号（アスタリスクはオプションで、前後に空白を許容）
term_separator = r"\s*\*?\s*"

# 3つの要素の全ての順列（6通り）を作成し、term_separatorで結合
# (e_base, sin_base, power_base)
perm1 = e_base_pattern + term_separator + sin_base_pattern + term_separator + power_base_pattern
# (e_base, power_base, sin_base)
perm2 = e_base_pattern + term_separator + power_base_pattern + term_separator + sin_base_pattern
# (sin_base, e_base, power_base)
perm3 = sin_base_pattern + term_separator + e_base_pattern + term_separator + power_base_pattern
# (sin_base, power_base, e_base)
perm4 = sin_base_pattern + term_separator + power_base_pattern + term_separator + e_base_pattern
# (power_base, e_base, sin_base)
perm5 = power_base_pattern + term_separator + e_base_pattern + term_separator + sin_base_pattern
# (power_base, sin_base, e_base)
perm6 = power_base_pattern + term_separator + sin_base_pattern + term_separator + e_base_pattern

# 全ての順列をOR条件で結合
all_permutations = f"(?:{perm1}|{perm2}|{perm3}|{perm4}|{perm5}|{perm6})"

pattern_definitions = {
    # eの微分: 2*e^(2*x)
    'eの微分': r"(?:2\s*\*?\s*(?:e\^\{?2\*?x\}?|exp\s*\(\s*2\*?x\s*\))|(?:e\^\{?2\*?x\}?|exp\s*\(\s*2\*?x\s*\))\s*\*?\s*2)",
    
    # 三角関数の微分: -3*x*sin(x^3+1)) のように、- と sin の間に任意の文字列を許容
    '三角関数の微分': r"-\s*.*?sin\s*\(\s*(?:x\^3\s*\+\s*1|1\s*\+\s*x\^3)\s*\)\)?" ,
    
    # 合成関数の微分: 一番最初に-があり、e^(2*x), sin(x^3+1), 3*x^2が*で結合（順不同）
    '合成関数の微分': r"^\-?\s*" + all_permutations + r"$" ,
    
    # 累乗の微分: 3*x^2
    '累乗の微分': r"(?:[0-9]*\.?[0-9]*\s*\*\s*)?3\*?x\^2",
}
...
try:
    ...
    for idx, row_data in df_processed.iterrows():
        response_to_check = str(row_data['extracted_ans1'])
        ...
        for pm_col_name, pattern_regex in pattern_definitions.items():
            if pd.notna(response_to_check) and re.search(pattern_regex, response_to_check):
    ...
    # -------------------------------------------------------------
    # パターンマッチング結果の列も確認対象に含める (日本語に簡潔化された列名を使用)
    columns_to_check_for_excel = ['extracted_ans1', 'extracted_cor1'] + list(pattern_definitions.keys()) + ['PM_その他正解', 'PM_その他誤答']
    ...
    if not final_output_df.empty:
        # 出力ファイル名を「4-5」と分かるように変更
        output_csv_filename_for_this_file = f"HW04-05_pattern_analysis_optimal_jp.csv" # ここを変更
